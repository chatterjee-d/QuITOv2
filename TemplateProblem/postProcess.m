...
%------------- BEGIN CODE --------------
if strcmp(options.meshstrategy,'fixed')
    quito_solution= solution;
elseif strcmp(options.meshstrategy,'mesh refinement')
...
if strcmp(options.meshstrategy,'mesh refinement') && options.MRplot==1
    figure;
    maroon= [0.6350 0.0780 0.1840];
    iter= length(solution.cost_arr)-1;
    hold on;
    grid on;
    plot(solution.solution_knot.tau, 1,'o','Color', maroon,'MarkerSize',4);
    for i = 1:iter
        plot(solution.tau_history{i}, i+1,'bo','MarkerSize',4);
    end
    ylabel('Iteration');
    yticks(1:1:iter+1);
    xlabel('Time');
